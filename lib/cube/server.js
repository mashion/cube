...
module.exports = function(options) {
  var server = {},
      ...
      id = 0,
      authFun = options.authentication,
      namespaceFun = options.namespace;
  ...
  server.start = function() {
    ...
    db.open(function(error) {
      if (options["mongo-username"] == null) return createNamespacesCollection();
    ...
    function ready() {
      ...
      server.register(db, endpoints, authFun, namespaceFun);
    ...
    function createNamespacesCollection() {
      if (namespaceFun) {
        db.createCollection("cube_namespaces", {}, function (error, namespaces) {
          if (error) throw error;
          ready();
        });
      } else {
