...
exports.putter = function(db, namespaceFun) {
  ...
  function putter(request, callback) {
    ...
    function namespaceFunCallback(type) {
      ...
      db.collectionNames(type + "_events", function(error, names) {
        ...
        function addTypeToNamespace() {
          if (!namespaceFun) return;

          namespaceFun("", request, function (ns) {
            db.collection("cube_namespaces").insert({_id: ns, types: []});
            db.collection("cube_namespaces").update({_id: ns}, {$push: { types: type }});
          });
        }
      });
    }
...
exports.getter = function(db, namespaceFun) {
  var collection = types(db),
      streamsBySource = {};
  ...
  function getter(request, callback) {
    ...
    // Namespace the type if necessary.
    if (namespaceFun) {
      namespaceFun(expression.type, request, namespaceFunCallback);
    } else {
      namespaceFunCallback(expression.type);
    }
