...
exports.register = function(db, endpoints, authFun) {
  var event = authentication(require("./event").getter(db)),
      metric = authentication(require("./metric").getter(db)),
      types = authentication(require("./types").getter(db));
  ...
  function authentication(getter) {
    return function(request, callback) {
      if (authFun && !authFun(request)) {
        throw "AuthenticationError: Invalid Credentials";
      }
      getter(request, callback);
    }
  }
};
